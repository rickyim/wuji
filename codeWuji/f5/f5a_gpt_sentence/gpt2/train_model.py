...
class GPT2TrainingSpec(TrainingSpec):
    ...
    def train_objective(self, data: Dict[str, torch.Tensor], model: nn.Module
        ...
        mask = (data['output'] != self.vocab.pad_idx).to(torch.int64)
        num_count = mask.sum()
        pred = logits.argmax(dim=2)
        pred = pred*mask
        correct = pred.eq((data['output']*mask).view_as(pred)).sum().item()
        correct = correct - (1-mask).sum()
        accuracy = correct / num_count
...
def train_gpt2_model(args: argparse.Namespace):
    ...
    config = TrainConfig(
        ...
        use_amp=args.use_amp, gpus=args.gpus, evaluate=args.evaluate)
...
def add_subparser(subparsers: argparse._SubParsersAction):
    ...
    group.add_argument('--dims', default=512, type=int,
    ...
    group.add_argument('--gpus', default=4, type=int,
                       help='number of gpu devices to use in training')
    group.add_argument('--evaluate', default=False, type=bool,
                       help='to evaluate')
