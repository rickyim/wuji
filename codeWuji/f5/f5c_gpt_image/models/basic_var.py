...
from utils.binlinear import BinLinear
...
try: from flash_attn import flash_attn_func              # type: ignore # qkv: BLHc, ret: BLHcq
except ImportError: pass
...
class FFN(nn.Module):
    def __init__(self, in_features, hidden_features=None, out_features=None, drop=0., fused_if_available=True):
        ...
        self.fc1 = BinLinear(in_features, hidden_features)
        self.act = nn.GELU(approximate='tanh')
        self.fc2 = BinLinear(hidden_features, out_features)
...
class SelfAttention(nn.Module):
    def __init__(
        ...
        self.proj = BinLinear(embed_dim, embed_dim)
    ...
    def forward(self, x, attn_bias):
        ...
        real_weight = self.mat_qkv.weight
        mean_weights = real_weight.mul(-1).mean(dim=1, keepdim=True).expand_as(self.mat_qkv.weight).contiguous()
        ...
        qkv = F.linear(input=x, weight=signed_weights, bias=torch.cat((self.q_bias, self.zero_k_bias, self.v_bias))).view(B, L, 3, self.num_heads, self.head_dim)
