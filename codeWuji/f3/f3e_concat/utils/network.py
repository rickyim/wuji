...
class Scan_ONN(nn.Module):
    def __init__(self,
        ...
        self.chip_concate_num = int(self.mask_pixel_num / 128)
    ...
    def forward(self, img, N, concate_num):
        
        measurement = []
        mask_voltage = self.mask_voltage % 2.5 + 0.5

        propad_concate = int((self.chip_concate_num - 14) * 128 * 5)
        for idx, lamda in enumerate(self.lambda_):
            ...
            hidden = img * mask
            for i in range(concate_num - 1):
                hidden[:, :, (i * (N + 1) + 1) * 128 : (i * (N + 1) + N + 1) * 128] = 0
            ...
            hidden = hidden[:, :, self.propad + propad_concate : - self.propad - propad_concate]
