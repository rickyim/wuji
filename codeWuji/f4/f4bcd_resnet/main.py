...
parser.add_argument('--std', dest='std', action='store_true',
                    help='whether to set std', default = False)
...
def main():
    ...
    model = get_model(args.arch, pretrained = args.pretrained, binarize=args.quantize, num_classes = args.num_classes)
    ...
    # Data loading code
    traindir = "/data/yizhou/ImageNet/train"
    valdir = "/data/yizhou/ImageNet/val"
    ...
    if args.evaluate:
        prec1 = validate(val_loader, model, criterion, args.start_epoch)
        return
...
def train(train_loader, model, criterion, optimizer, epoch):
    ...
    model.train()
    print(' * lr {}'.format(args.lr * (0.1 ** (epoch // 30))))
    end = time.time()
...
def save_checkpoint(state, is_best, filename='checkpoint.pth.tar'):
    filename = 'checkpoint_%s.xc_wc.pth.tar'%(args.arch)
    torch.save(state, filename)
    if is_best:
        shutil.copyfile(filename, "model_best_%s.xc_wc.pth.tar"%(args.arch))
